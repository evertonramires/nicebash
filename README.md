...
- For long-running commands, `stamp <command>` prefixes **every** output line with its arrival time, e.g. `stamp ping 8.8.8.8`. Stamping auto-disables when the output is piped or redirected, so `stamp ping 8.8.8.8 | grep ttl` still receives clean data
